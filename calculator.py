...
class CalculatorApp:
    ...
    def show_history(self):
        ...
        main_frame = ctk.CTkScrollableFrame(history_window, bg_color="#d4d4d8", fg_color="#d4d4d8")
        ...
        for i, (expr, result) in enumerate(self.histories):
            expr_label = ctk.CTkButton(main_frame, text=f'{expr} = ', width=0, font=('Helvetica', 12, 'bold'), command=lambda x=expr: button_action(x), text_color='#27272a', fg_color="#d4d4d8")
            expr_label.grid(row=i, column=0, pady=2, sticky='e')
            result_button = ctk.CTkButton(main_frame, text=result, width=0, font=('Helvetica', 12, 'bold'), command=lambda x=result: button_action(x), text_color='#27272a', fg_color="#d4d4d8") # type: ignore
    ...
    def setup_ui(self):
        ...
        # Label
        button_history = ctk.CTkButton(history_expression_frame, text='History', anchor='w', width=0,font=('Helvetica', 12, 'bold'), fg_color="#e4e4e7", text_color="#27272a", command=self.show_history)
        ...
        for button in buttons:
            btn = ctk.CTkButton(button_frame, text=button, font=('Helvetica', 16, 'bold'), fg_color="#e4e4e7", text_color="#27272a", command=lambda x=button: self.button_action(x))
            if button == '0':
